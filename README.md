...
1. Créez un nouveau dépôt en cliquant sur le bouton `Use this template` en haut à droite, puis `Create a repository`.
2. Une fois sur l'interface de création du dépôt, **cochez** `Include all branches`, donnez un nom à votre dépôt (`l3m-git-2026-2027` par exemple), puis cliquez sur le bouton `Create repository` en bas à droite. Vous devriez maintenant avoir votre propre dépôt à cette adresse : `https://github.com/<votre-compte-github>/<nom-de-votre-depot>` (par exemple, avec un compte appelé `l3m-billyx` et un dépôt appelé `super-tp-git`, l'adresse serait `https://github.com/l3m-billyx/super-tp-git`)
